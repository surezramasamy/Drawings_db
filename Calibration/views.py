...
from .models import Instruments_List,Fixture_List,Template_List
...
def cal(request):    
    ...
    queryset4=Fixture_List.objects.order_by("Next_Verification_Due").annotate(Actual=Count('id'))
    x4 = list(queryset4.values_list('Next_Verification_Due', flat=True))
    y4 = list(queryset4.values_list('Actual', flat=True))
    df7 = pd.DataFrame(queryset4.values("Next_Verification_Due","Actual"))
    df7['month2'] = pd.to_datetime(df7['Next_Verification_Due']).dt.to_period('M')
    df8 = df7.to_html()


    df4=pd.concat([df, df2]).groupby(['month2']).sum(numeric_only=True).reset_index()
    df6=df4.set_index('month2')
    df5 = df6.to_html()
    graph = get_plot2(df4)
    
    df9=pd.concat([df7, df2]).groupby(['month2']).sum(numeric_only=True).reset_index()
    df10=df9.set_index('month2')
    df11 = df10.to_html()
    graph1 = get_plot2(df9)
...
def cal1(request):    
    ...
    queryset = Template_List.objects.order_by('Status').values('Status').annotate(count=Count('Description'))
    y = list(queryset.values_list('count', flat=True))
    x = list(queryset.values_list('Status', flat=True))
    chart=get_plot(x,y)

    queryset1 = Template_List.objects.order_by('Location_used').values('Location_used').annotate(count=Count('Description'))
    y1 = list(queryset1.values_list('count', flat=True))
    x1 = list(queryset1.values_list('Location_used', flat=True))
    chart1=get_plot1(x1,y1)

    queryset5 = Template_List.objects.order_by('Location_used').values('Location_used').annotate(Fixtures=Count('Status'))
    dframe1 = pd.DataFrame(queryset5.values("Location_used","Status","Fixtures"))
    dframe2=dframe1.pivot(index='Location_used', columns='Status').fillna(0)
    dframe1 = dframe1.to_html()
    dframe4 = dframe2.to_html()
    ...
    queryset2=Template_List.objects.order_by("Verification_Plan").annotate(Plan=Count('id'))
    x2 = list(queryset2.values_list('Verification_Plan', flat=True))
    y2 = list(queryset2.values_list('Plan', flat=True))
    df = pd.DataFrame(queryset2.values("Verification_Plan","Plan"))
    df['month2'] = pd.to_datetime(df['Verification_Plan']).dt.to_period('M')
    ...
    queryset3=Template_List.objects.order_by("Verified_on").annotate(Actual=Count('id'))
    x3 = list(queryset3.values_list('Verified_on', flat=True))
    y3 = list(queryset3.values_list('Actual', flat=True))
    df2 = pd.DataFrame(queryset3.values("Verified_on","Actual"))
    df2['month2'] = pd.to_datetime(df2['Verified_on']).dt.to_period('M')
    ...
    qs4 = Template_List.objects.order_by('Verification_Freq').values('Verification_Freq').annotate(count=Count('Description'))
    qs5 = pd.DataFrame(qs4.values("Verification_Freq","count"))
    df7=qs5.set_index("Verification_Freq")
    df8 = df7.to_html()
    graph2 = get_plot3(df7)
    ...
    return render(request,"cal1.html",context)
...
def cal2(request):
    ...
    queryset5 = Instruments_List.objects.order_by('Location_used').values('Location_used').annotate(Instruments=Count('Status'))
    dframe1 = pd.DataFrame(queryset5.values("Location_used","Status","Instruments"))
    dframe2=dframe1.pivot(index='Location_used', columns='Status').fillna(0)
    dframe1 = dframe1.to_html()
    dframe4 = dframe2.to_html()
    graph5 = get_plot4(dframe2)
...
class   Templates_Listview(ListView):
    from dateutil.relativedelta import relativedelta
    ...
    now = datetime.datetime.now()
    after = now + datetime.timedelta(days=30)
    queryset = Template_List.objects.filter(Next_Verification_Due__lte = after)
    ordering = ['Next_Verification_Due']

class Templates_Listview2(ListView):
    context_object_name = 'Templates2'
    model = Template_List
    template_name=  'Templates2.html'
    queryset =Template_List.objects.filter(Q(Status = "Conditionally_accepted") | Q(Status = "Rejected") )
    ordering = ['Status']
