...
def get_plot(x,y):
    ...
    plt.title("Fixture_or_Template Verification Status")
    plt.bar(x,y)
    addlabels(x,y)
...
def get_plot1(x1,y1):
    plt.switch_backend ('AGG')
    plt.figure(figsize=(5,4))
    plt.title("Locationwise Fixtures_or_Templates")
    plt.bar(x1,y1)
    addlabels1(x1,y1)
    plt.xticks(rotation=45)
...
def get_plot2(df4):
    ...
    plt.switch_backend ('AGG')
    ax = df4.plot.bar(x='month2',title="Monthwise Plan vs Actual",figsize=(4,4),linewidth=1)
